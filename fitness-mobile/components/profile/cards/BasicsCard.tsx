...
export default memo(function BasicsCard(props: Props) {
  ...
  return (
    <Animated.View entering={FadeInDown.duration(240)} style={{ gap: 18 }}>
      {/* Sex */}
      <View style={{ flexDirection: "row", gap: 10 }}>
        {(["male", "female"] as const).map((s) => {
          const active = props.sex === s;
          return (
            <Pressable
              key={s}
              onPress={() => {
      ...
      <View style={{ flexDirection: "row", gap: 12 }}>
        <Input
          label="Age"
          value={props.age}
          onChange={(t) => props.setAge(onlyInt(t))}
          suffix="yrs"
        />
        <Input
          label="Height"
          value={props.heightCm}
          onChange={(t) => props.setHeightCm(onlyInt(t))}
          suffix="cm"
        />
      </View>
      ...
      {/* Weight */}
      <Input
        label={`Weight (${props.weightUnit.toUpperCase()})`}
      ...
      <View style={{ flexDirection: "row", gap: 10 }}>
        {(["kg", "lb"] as const).map((u) => {
          const active = props.weightUnit === u;
          return (
            <Pressable
              key={u}
              onPress={() => {
                Haptics.selectionAsync();
                props.setWeightUnit(u);
              }}
              style={{
                flex: 1,
                paddingVertical: 10,
