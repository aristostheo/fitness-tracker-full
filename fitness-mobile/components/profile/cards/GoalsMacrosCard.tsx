...
// components/profile/cards/GoalsMacrosCard.tsx
...
type Props = {
  ...
  setWeeklyPace: (v: string) => void;
  aggressionPct: number;
  setAggressionPct: (v: number) => void;
  calorieCyclingPct: number;
  setCalorieCyclingPct: (v: number) => void;

  macroMethod: MacroMethod;
  setMacroMethod: (v: MacroMethod) => void;
  proteinPerKg: string;
  setProteinPerKg: (v: string) => void;
  proteinPct: number;
  setProteinPct: (v: number) => void;
  carbPct: number;
  setCarbPct: (v: number) => void;
  fatPct: number;
  setFatPct: (v: number) => void;
  trainCarbPct: number;
  setTrainCarbPct: (v: number) => void;
  restCarbPct: number;
  setRestCarbPct: (v: number) => void;

  onPreview: (p: any) => void;
};

export default function GoalsMacrosCard(props: Props) {
  const { colors } = useTheme();
  ...
  return (
    <Animated.View entering={FadeInDown.duration(260)} style={{ gap: 24 }}>
      {/* Goal */}
      <Section title="Goal">
        <View style={{ flexDirection: "row", gap: 10 }}>
          {(["maintain", "cut", "bulk"] as GoalType[]).map((g) => (
            <Chip
              key={g}
              label={g.toUpperCase()}
              active={props.goalType === g}
              onPress={() => props.setGoalType(g)}
            />
          ))}
        </View>
      </Section>
      ...
      <Section
        title="Activity"
        subtitle="Used to calculate calories & recovery"
      >
        <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}>
          {(
            ...
            <Chip
              key={a}
              label={a}
              active={props.activityLevel === a}
              onPress={() => props.setActivityLevel(a)}
            />
          ))}
        </View>
      </Section>
      ...
      <Section title="Macros">
        <Chip
          label={showMacros ? "Hide macro tuning" : "Customize macros"}
          active={showMacros}
      ...
      {showMacros && (
        <Section title="Macro method" subtitle="Advanced users only — defaults work great">
          <View style={{ flexDirection: "row", gap: 10 }}>
            {(["proteinPerKg", "percent", "cycling"] as MacroMethod[]).map(
              (m) => (
                <Chip
                  key={m}
                  label={m}
                  active={props.macroMethod === m}
                  onPress={() => props.setMacroMethod(m)}
                />
              )
            )}
          </View>
