...
export default function ProfileScreen() {
  ...
  const lastWeightKgRef = useRef<number | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      unsub = subscribeProfile(user.uid, (p) => {
        ...
        lastWeightKgRef.current = Number.isFinite(wkg) ? wkg : null;
        setWeightInput(
          unit === "lb"
  ...
  async function onSave(manual = true) {
    ...
    try {
      ...
      const prevWeightKg = lastWeightKgRef.current;
      const newWeightKg = weightKg;
      const weightChanged =
        Number.isFinite(newWeightKg) &&
        newWeightKg > 0 &&
        (prevWeightKg == null || Math.abs(newWeightKg - prevWeightKg) > 0.01);
      ...
      // Log weight history if the user changed weight
      if (weightChanged) {
        try {
          ...
          lastWeightKgRef.current = newWeightKg;
        } catch {}
      } else if (newWeightKg > 0) {
        lastWeightKgRef.current = newWeightKg;
      }
      ...
      <View style={{ flex: 1 }}>
        ...
        <Animated.View style={{ flex: 1, opacity: fadeIn }}>
          ...
          >
                ...
                >
                  ...
                  <View style={{ flex: 1, gap: 4 }}>
                    ...
                    <Text style={{ color: colors.muted, fontWeight: "600" }}>
                      Get AI meals, workout generator, templates, and more.
                      Admin/tester users always have full access.
                    </Text>
                  </View>
                  <Pressable
            ...
            {/* Profile progress + jumps */}
            <LinearGradient
              colors={[withAlpha(colors.primary, 0.18), colors.card]}
              start={{ x: 0, y: 0 }}
              end={{ x: 1, y: 1 }}
              style={{
                borderRadius: 18,
            ...
            >
              ...
              >
                <View style={{ flex: 1, minWidth: 0 }}>
                  ...
                  >
                    Complete your basics, goals, and meals to personalize
                    targets.
                  </Text>
                </View>
                <View
              ...
              <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}>
                {[
                  { label: "Basics", icon: "person-outline", key: "basics" },
                  { label: "Goals", icon: "trophy-outline", key: "goals" },
                  { label: "Meals", icon: "fast-food-outline", key: "meals" },
                ].map((item) => (
                  <Pressable
                    key={item.key}
                    onPress={() =>
                      openAndJump(item.key as keyof typeof anchors)
                    }
                  >
                    {({ pressed }) => (
                      <MotiView
...
function HeroHeader({
  ...
  return (
    ...
    >
      <LinearGradient
        colors={[withAlpha(colors.primary, 0.25), withAlpha(colors.card, 0.98)]}
        start={{ x: 0, y: 0 }}
        end={{ x: 1, y: 1 }}
        style={{
          borderRadius: 22,
      ...
      >
        ...
        >
          ...
          />
          {/* Theme toggle floating to preserve space */}
          <View
            style={{
              position: "absolute",
              top: 14,
          ...
          >
            ...
            <View style={{ flex: 1, gap: 4 }}>
              ...
              <Text style={{ color: colors.muted }} numberOfLines={1}>
                Personalization, macros, and goals in one place.
              </Text>
            </View>
          </View>
          ...
          >
            ...
            />
            <Pill icon="flag-outline" label="Goal" value={String(goal)} />
            <Pill
              icon="flash-outline"
              label="Activity"
...
function TargetsPreview({
  ...
  const tile = (
    ...
    >
      ...
      >
        <Text
          style={{ color: colors.text, fontWeight: "900", fontSize: 24 }}
          numberOfLines={1}
        >
          {formatVal(value)}
        </Text>
...
function QuickActions({
  ...
  const actions = [
    ...
    { icon: "sync-outline" as const, label: "Sync health", onPress: onSync },
    { icon: "download-outline" as const, label: "Export", onPress: onExport },
    {
      icon: "refresh-outline" as const,
      label: "Reset targets",
      onPress: onReset,
    },
  ].filter((a) => !!a.onPress) as {
    icon: keyof typeof Ionicons.glyphMap;
    label: string;
    onPress: () => void;
  }[];
