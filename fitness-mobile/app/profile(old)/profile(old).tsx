...
import { BodyTwinCard } from "@/components/scanMeal/new/bodyTwin/BodyTwinCard";
...
export default function ProfileScreen() {
  ...
  const handlePreview = useCallback(
    (t: Targets, meta?: { userInitiated?: boolean }) => {
      // ✅ ignore previews fired on mount/open unless user actually changed something
  ...
  const bodyTwinMetrics = useMemo(
    () => ({
      updatedAt: Date.now(),
      weightKg,
      heightCm: Number(heightCm || 0),
      // Optional: if you have these in Profile later, add them:
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* ✅ Body Twin (best placement: identity -> goals) */}
        {/* <BodyTwinCard currentMetrics={bodyTwinMetrics} /> */}

        <TargetsGlassPanel
          preview={macrosPreview}
          fallback={{
            calories: savedTargets.calorieGoal,
            protein: savedTargets.proteinGoal,
            carbs: savedTargets.carbGoal,
            fat: savedTargets.fatGoal,
          }}
        />
        ...
        >
          <AdvancedGoalsEngineCard
            ...
            age={Number(age || 0)}
            heightCm={Number(heightCm || 0)}
            // ✅ NEW: last saved goals to initialize sliders/inputs
            savedTargets={savedTargets}
            // ✅ only updates preview when user changes something
